...
class CompoundInterestCalculatorProvider extends GetxController {
  ...
  final _rate = 0.0.obs;
  final _time = 0.0.obs;
  final _compoundingFrequency = 1.obs;
  ...
  void calculate() {
    if (_principal.value > 0 && _rate.value > 0 && _time.value > 0) {
      double amount = _principal.value * pow((1 + (_rate.value / (100 * _compoundingFrequency.value))), (_compoundingFrequency.value * _time.value));
      _totalAmount.value = amount;
      _interest.value = amount - _principal.value;
    } else {
      _interest.value = 0;
